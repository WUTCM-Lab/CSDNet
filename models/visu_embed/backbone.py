...
class Joiner(nn.Sequential):
    def __init__(self, backbone, position_embedding, down_sample=False, down_dim=256):
        super().__init__(backbone, position_embedding)
        if down_sample:
            self.down = nn.Conv2d(backbone.num_channels, down_dim, kernel_size=1)
        else:
            self.down = None
    ...
    def forward(self, tensor_list: NestedTensor):
        ...
        # Need keyword correction with A
        if self.down is not None: 
            xs['3'] = self.down(xs['2'])  # the downsampling continues for the last layer
